...
public class EntityRecorder {
    ...
    public void startRecording(PlayerEntity player, MinecraftClient client) {
        ...
        if (client.targetedEntity instanceof LivingEntity) {
            LivingEntity entity = (LivingEntity) client.targetedEntity;
            this.target = entity;
            this.player = player;
    ...
    public static void rendererTick(MatrixStack matrices, float tickDelta) {
        ...
        Framebuffer framebuffer = new Framebuffer(scaledWidth, scaledHeight, true, MinecraftClient.IS_SYSTEM_MAC);
        framebuffer.beginWrite(true);
        RenderSystem.disableBlend();

        //旋转模型
        RenderSystem.pushMatrix();
        RenderSystem.rotatef(45f, 0, -1, 0);
